...
import { coincidentPoint2, dotProductPoint2, DOWN, RIGHT, sumPoint2, TURN_LEFT, TURN_NONE, TURN_RIGHT } from '@advent-of-code/utils/points'
...
export function solve () {
  ...
  Q.push(new State(0, start, DOWN, 0))
  Q.push(new State(0, start, RIGHT, 0))
  ...
  while (Q.size()) {
    ...
    if (coincidentPoint2(pos, end) && dist >= 4) {
      return loss
    }
    ...
    for (const turn of [TURN_NONE, TURN_LEFT, TURN_RIGHT]) {
      ...
      if (grid.has(nextPos)) {
        if ((sameDir && dist < 10) || (!sameDir && dist >= 4)) {
